...
const api = {
  async get(url) {
    try {
      ...
      return r.json();
    } catch (e) { showToast(e.message || 'Network error', 'error'); throw e; }
  },
  async post(url, data) {
    try {
      ...
      if (!r.ok) { const e = await r.json().catch(() => ({ error: r.statusText })); throw new Error(e.error || r.statusText); }
      return r.json();
    } catch (e) { showToast(e.message || 'Request failed', 'error'); throw e; }
  },
  async put(url, data) {
    try {
  ...
  async del(url, data) {
    try {
      ...
      if (!r.ok) { const e = await r.json().catch(() => ({ error: r.statusText })); throw new Error(e.error || r.statusText); }
      return r.json();
    } catch (e) { showToast(e.message || 'Delete failed', 'error'); throw e; }
  }
};
...
function spinner(size = 40) { return `<div class="spinner" style="width:${size}px;height:${size}px"></div>`; }
function loadingCard(msg = 'Loading...') { return `<div class="card mt-16" style="text-align:center;padding:40px">${spinner()}<p class="text-dim mt-8">${escHtml(msg)}</p></div>`; }
function timeAgo(d) {
  if (!d) return '-';
...
function lineChart(points, w = 600, h = 180) {
  if (!points.length) return '<div class="text-dim text-sm" style="padding:20px;text-align:center">No data yet — run an audit to start tracking</div>';
...
// ─── Stored quick-check result for XSS-safe passing ───
let _lastQuickCheckResult = null;

// ─── DASHBOARD ───
async function renderDashboard() {
  app.innerHTML = `<h1 class="page-title">Dashboard</h1><p class="page-subtitle">Loading...</p>${loadingCard()}`;
  try {
    ...
    // If no site exists, show the URL-first hero experience
    const heroSection = !site ? `
      <div class="hero-card mb-24">
        <div class="hero-icon">
          <svg width="48" height="48" viewBox="0 0 48 48" fill="none"><circle cx="24" cy="24" r="20" stroke="var(--green)" stroke-width="2.5"/><path d="M18 24.5l4 4 8-8" stroke="var(--green)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </div>
        <h2>Check your website's SEO health</h2>
        <p class="text-dim">Paste your URL below and get an instant SEO audit with actionable fixes</p>
        <div class="hero-input-group">
          <input class="form-input hero-input" id="heroUrl" placeholder="https://yourwebsite.com" autofocus>
          <button class="btn btn-primary btn-lg" onclick="runHeroCheck()">Analyze</button>
        </div>
        <div id="heroResult"></div>
      </div>
    ` : `
      <div class="quick-check-bar mb-24">
        <input class="form-input" id="heroUrl" placeholder="Enter URL for quick SEO check..." value="${escHtml(site.url || '')}">
        <button class="btn btn-primary" onclick="runHeroCheck()">Quick Check</button>
        ${site ? `<button class="btn btn-secondary" onclick="reauditSite()">Re-audit ${escHtml(site.domain)}</button>` : ''}
      </div>
      <div id="heroResult"></div>
    `;
    ...
    app.innerHTML = `
      ...
      <p class="page-subtitle">${site ? `Monitoring ${escHtml(site.domain)}` : 'Get started with your first SEO audit'}</p>
      ${heroSection}
      ${site ? `
        ...
        <div class="card">
          ...
          ${recentIssues.length ? recentIssues.map(i => `
            <div class="audit-issue">
              ...
              <div style="flex:1"><div class="fw-600">${escHtml(i.title)}</div><div class="text-dim text-sm">${escHtml(i.category)} &middot; ${timeAgo(i.createdAt)}</div></div>
              ${severityBadge(i.severity)}
            </div>
          `).join('') : '<div class="empty-state-sm"><p class="text-dim">No open issues — run an audit to discover them</p></div>'}
        </div>
      </div>` : ''}`;
  } catch (e) {
    app.innerHTML = `<h1 class="page-title">Dashboard</h1><div class="card"><p class="text-red">Failed to load dashboard: ${escHtml(e.message)}</p></div>`;
  }
}

// Hero/dashboard quick check
window.runHeroCheck = async function() {
  const urlInput = $('#heroUrl');
  ...
  const el = $('#heroResult');
  el.innerHTML = loadingCard('Analyzing your website...');
  try {
    ...
    _lastQuickCheckResult = r;
    renderQuickCheckResult(el, r);
  } catch (e) {
    el.innerHTML = `<div class="card mt-16"><p class="text-red">Error: ${escHtml(e.message)}</p></div>`;
  }
};

window.reauditSite = async function() {
  try {
...
function renderQuickCheckResult(el, r) {
  ...
  el.innerHTML = `<div class="card mt-16">
    ...
    `).join('')).join('')}
    ${r.totalIssues === 0 ? '<p class="text-dim" style="padding:12px 0">No issues found — great job!</p>' : ''}
    <div class="mt-16"><button class="btn btn-primary" onclick="saveLastQuickCheck()">Save as Audit & Track Issues</button></div>
  </div>`;
}

window.saveLastQuickCheck = async function() {
  const result = _lastQuickCheckResult;
  if (!result) return;
  try {
    const sites = await api.get('/api/sites');
    ...
    showToast('Audit saved and issues created!');
    _lastQuickCheckResult = null;
...
async function renderAudits() {
  app.innerHTML = `<h1 class="page-title">Audits</h1>${loadingCard()}`;
  try {
    ...
    app.innerHTML = `
      ...
      <div class="quick-check-bar">
        <input class="form-input" id="quickCheckUrl" placeholder="Enter URL for quick SEO check..." value="${escHtml(site?.url || '')}">
        <button class="btn btn-primary" onclick="runQuickCheck()">Run Audit</button>
      </div>
      ...
      <div class="card mt-16">
        ...
        </tbody></table>` : '<div class="empty-state"><p>No audits yet. Enter a URL above and run your first audit.</p></div>'}
      </div>`;
  } catch (e) {
    app.innerHTML = `<h1 class="page-title">Audits</h1><div class="card"><p class="text-red">Failed to load: ${escHtml(e.message)}</p></div>`;
  }
}

window.runQuickCheck = async function() {
  let url = $('#quickCheckUrl')?.value?.trim();
  if (!url) return;
  if (!url.match(/^https?:\/\//)) url = 'https://' + url;
  $('#quickCheckUrl').value = url;
  const el = $('#quickCheckResult');
  el.innerHTML = loadingCard('Analyzing website...');
  try {
    const r = await api.post('/api/audits/quick-check', { url });
    if (r.error) { el.innerHTML = `<div class="card mt-16"><p class="text-red">${escHtml(r.error)}</p></div>`; return; }
    _lastQuickCheckResult = r;
    renderQuickCheckResult(el, r);
  } catch (e) { el.innerHTML = `<div class="card mt-16"><p class="text-red">Error: ${escHtml(e.message)}</p></div>`; }
};
...
async function renderAuditDetail(id) {
  ...
  try {
    ...
    app.innerHTML = `<div class="card"><p class="text-red">Failed to load audit: ${escHtml(e.message)}</p></div>`;
  }
}

// ─── ISSUES ───
async function renderIssues() {
  app.innerHTML = `<h1 class="page-title">Issues</h1>${loadingCard()}`;
  try {
        ...
        ${issues.length === 0 ? `
          <div class="empty-state-card card">
            <div class="empty-state">
              <svg width="48" height="48" viewBox="0 0 48 48" fill="none"><path d="M24 8l16 28H8L24 8z" stroke="var(--text-muted)" stroke-width="2"/><path d="M24 20v6M24 30v1" stroke="var(--text-muted)" stroke-width="2" stroke-linecap="round"/></svg>
              <p>No issues tracked yet</p>
              <p class="text-sm">Run an audit from the <a href="#dashboard">Dashboard</a> to automatically discover SEO issues, or add one manually.</p>
            </div>
          </div>
        ` : `
    ...
    app.innerHTML = `<h1 class="page-title">Issues</h1><div class="card"><p class="text-red">Failed to load: ${escHtml(e.message)}</p></div>`;
  }
}
...
window.saveNewIssue = async function() {
  ...
  try {
    const sites = await api.get('/api/sites');
    await api.post('/api/issues', {
      siteId: sites[0]?.id || '', title, description: $('#issueDesc').value,
...
async function renderContent() {
  app.innerHTML = `<h1 class="page-title">Content Studio</h1>${loadingCard()}`;
  try {
    ...
    app.innerHTML = `
      ...
      ${content.length ? `<div class="card mb-20"><div class="table-wrap"><table><thead><tr><th>Title</th><th>Type</th><th>SEO Score</th><th>Keywords</th><th>Status</th><th>Words</th><th>Date</th></tr></thead><tbody>
        ...
        <div class="empty-state-card card mb-20">
          <div class="empty-state">
            <p>No content yet</p>
            <p class="text-sm text-dim">Create your first SEO-optimized article, landing page, or FAQ.</p>
            <button class="btn btn-primary mt-8" onclick="createNewContent()">Create Content</button>
          </div>
        </div>
      `}
      <div class="card">
    ...
    app.innerHTML = `<h1 class="page-title">Content Studio</h1><div class="card"><p class="text-red">Failed to load: ${escHtml(e.message)}</p></div>`;
  }
}
...
async function renderContentEditor(id) {
  ...
  try {
    ...
    app.innerHTML = `<div class="card"><p class="text-red">Failed to load editor: ${escHtml(e.message)}</p></div>`;
  }
}

// ─── KEYWORDS ───
async function renderKeywords() {
  app.innerHTML = `<h1 class="page-title">Keywords</h1>${loadingCard()}`;
  try {
    ...
    function render() {
      ...
      app.innerHTML = `
        ...
        ${keywords.length === 0 ? `
          <div class="empty-state-card card">
            <div class="empty-state">
              <svg width="48" height="48" viewBox="0 0 48 48" fill="none"><circle cx="20" cy="20" r="12" stroke="var(--text-muted)" stroke-width="2"/><path d="M29 29l10 10" stroke="var(--text-muted)" stroke-width="2" stroke-linecap="round"/></svg>
              <p>No keywords tracked yet</p>
              <p class="text-sm text-dim">Add your first keyword to start tracking rankings and discover opportunities.</p>
              <button class="btn btn-primary mt-8" onclick="showAddKeywordModal()">Track Keyword</button>
            </div>
          </div>
        ` : `
        <div class="tabs">
    ...
    window.deleteKeyword = async function(id) {
      const ok = await confirmAction('Delete this keyword?');
      if (!ok) return;
      try {
        await api.del('/api/keywords', { id });
        const idx = keywords.findIndex(k => k.id === id); if (idx > -1) keywords.splice(idx, 1);
    ...
    window.saveKeyword = async function() {
      const kw = $('#kwKeyword').value;
      if (!kw.trim()) { showToast('Keyword is required', 'error'); return; }
      try {
        const sites = await api.get('/api/sites');
        const newKw = await api.post('/api/keywords', {
          siteId: sites[0]?.id || '', keyword: kw,
    ...
    app.innerHTML = `<h1 class="page-title">Keywords</h1><div class="card"><p class="text-red">Failed to load: ${escHtml(e.message)}</p></div>`;
  }
}

// ─── LINKS ───
async function renderLinks() {
  app.innerHTML = `<h1 class="page-title">Links</h1>${loadingCard()}`;
  try {
    ...
    function render() {
      ...
      app.innerHTML = `
        ...
        ${links.length === 0 ? `
          <div class="empty-state-card card">
            <div class="empty-state">
              <svg width="48" height="48" viewBox="0 0 48 48" fill="none"><path d="M20 28l8-8M14 26l-2 2a6 6 0 008.4 8.4l2-2M34 22l2-2a6 6 0 00-8.4-8.4l-2 2" stroke="var(--text-muted)" stroke-width="2" stroke-linecap="round"/></svg>
              <p>No links tracked yet</p>
              <p class="text-sm text-dim">Add a backlink to start monitoring your link profile and discover opportunities.</p>
              <button class="btn btn-primary mt-8" onclick="showAddLinkModal()">Add Link</button>
            </div>
          </div>
        ` : `
        <div class="tabs">
    ...
    window.deleteLink = async function(id) {
      ...
      try {
        ...
        const idx = links.findIndex(l => l.id === id); if (idx > -1) links.splice(idx, 1);
    ...
    app.innerHTML = `<h1 class="page-title">Links</h1><div class="card"><p class="text-red">Failed to load: ${escHtml(e.message)}</p></div>`;
  }
}

// ─── INTEGRATIONS (renamed to Setup Guides) ───
async function renderIntegrations() {
  app.innerHTML = `<h1 class="page-title">Setup Guides</h1>${loadingCard()}`;
  try {
    ...
    function render() {
      if (selectedGuide) {
        ...
        app.innerHTML = `
          <div class="flex-between mb-20">
            <div><h1 class="page-title">${g.name}</h1><p class="page-subtitle">${g.type} — setup guide</p></div>
            <button class="btn btn-secondary" onclick="selectedGuide=null;renderIntegrations()">Back</button>
          </div>
          <div class="info-banner mb-16">ℹ️ These are step-by-step setup instructions. Connect to these platforms externally, then use their data with your SEO Command Center.</div>
          <div class="grid-2">
            <div class="card">
              <h3>Setup Steps</h3>
      ...
      app.innerHTML = `
        ...
        <p class="page-subtitle">Step-by-step instructions for connecting your SEO tools and CMS platforms</p>
        <div class="info-banner mb-16">📖 These are configuration guides — follow the steps to set up each platform, then enter your API keys in <a href="#settings">Settings</a>.</div>
        ...
        <div class="grid-3 mb-24">${guides.filter(g => g.type !== 'cms').map(g => `
          <div class="integration-card" onclick="selectedGuide='${g.platform}';renderIntegrations()">
            ...
            <div class="ic-type">Setup Guide</div>
          </div>
        `).join('')}</div>
    ...
    app.innerHTML = `<h1 class="page-title">Setup Guides</h1><div class="card"><p class="text-red">Failed to load: ${escHtml(e.message)}</p></div>`;
  }
}

// ─── SETTINGS ───
async function renderSettings() {
  app.innerHTML = `<h1 class="page-title">Settings</h1>${loadingCard()}`;
  try {
    ...
    app.innerHTML = `<h1 class="page-title">Settings</h1><div class="card"><p class="text-red">Failed to load: ${escHtml(e.message)}</p></div>`;
  }
}
...
window.confirmClearAll = async function() {
  const ok = await confirmAction('This will delete ALL sites, audits, issues, content, keywords, and links. Are you sure?');
  if (!ok) return;
  try {
    for (const ep of ['sites', 'audits', 'issues', 'content', 'keywords', 'links']) {
      const items = await api.get(`/api/${ep}`);
      if (items.length) {
        await api.post('/api/bulk-delete', { resource: ep, ids: items.map(i => i.id) });
      }
    }
    await api.put('/api/config', {});
